...
class SoyPortletRouter extends State {
	...
	createDefaultScreen_() {
		...
		class DeferredComponentScreen extends Router.defaultScreen {
			...
			/**
			 * Overrides the default ComponentScreen load method to continue
			 * only when the controller for the path and all its dependencies
			 * have been successfully loaded. If the load fails or no route is
			 * successfully found, then it falls back to an ordinary full page
			 * navigation
			 * @param {string} path
			 ...
			 */
			load(path) {
				const route = Router.router().findRoute(path);

				if (route !== undefined) {
					return new CancellablePromise((resolve, reject) => {
						super.load(path).then(loadedState => {
							const parsedState = JSON.parse(loadedState);
							Liferay.Loader.require(
								parsedState.controllerPath,
								module => {
									route.router.component = module.default;
									resolve(loadedState);
								},
								() => {
					...
					// Since we don't have the controllers, we can't assume we
					// can simply navigate and expect everything to be there. To
					// be safe, we need to do a proper navigation
	...
	createRouter_(route, pathFn) {
		...
		// TO-DO Can this really throw an exception?
		try {
	...
	initializeDefaultRouter_() {
		...
		this.defaultRouter_ = this.createRouter_(
			{
