...
public class UpgradeDynamicDataMapping extends UpgradeProcess {
		...
		try (LoggingTimer loggingTimer = new LoggingTimer();
			...
			PreparedStatement ps2 = connection.prepareStatement(
				"update DDMStructure set definition = ? where structureId = " +
					"?");
			PreparedStatement ps3 = connection.prepareStatement(sb.toString());
			PreparedStatement ps4 = connection.prepareStatement(sb2.toString());
			ResultSet rs = ps.executeQuery()) {
	...
	protected void upgradeTemplatesAndAddTemplateVersions() throws Exception {
		...
		try (LoggingTimer loggingTimer = new LoggingTimer();
			...
			PreparedStatement ps4 = connection.prepareStatement(sb.toString());
			ResultSet rs = ps.executeQuery()) {
