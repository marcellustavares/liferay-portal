...
public class EmbeddedElasticsearchServer {
	...
	public void pushMapping(String indexName, String mappingName)
		...
		String path =
			EmbeddedElasticsearchServer.class.getResource(
				"/META-INF/mappings/" + mappingName).getPath();
	...
	public SearchHits searchDocument(
		...
		searchRequestBuilder.setQuery(termQuery);
	...
	private EmbeddedElasticsearchServer(
		...
		nodeBuilder.settings(_settings);
	...
	private SearchHits searchDocument(
		...
		getClient().admin().indices().prepareRefresh(index).execute().
		actionGet();

		SearchResponse response =
			getClient().prepareSearch(index).setTypes(type).setQuery(termQuery).
			execute().actionGet();
